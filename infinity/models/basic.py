...
class SelfAttention(nn.Module):
    ...
    def kv_cache_shrink(self, batch_size: int):
        """Keep only the active samples when classifier-free branches are removed."""
        if self.cached_k is not None:
            self.cached_k = self.cached_k[:batch_size]
            self.cached_v = self.cached_v[:batch_size]
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        # ScaleErasure only enables this during inference. Keeping the query
        # cache opt-in avoids changing the normal Infinity execution path.
        self.capture_queries = False
        self.last_query_heads = None
    ...
    def forward(self, q, ca_kv):
        ...
        if self.capture_queries:
            self.last_query_heads = q_compact.view(
                B, Lq, self.num_heads, self.head_dim
            ).detach()
        else:
            self.last_query_heads = None
    ...
    def set_query_capture(self, enabled: bool) -> None:
        """Enable or disable the lightweight query cache used by ScaleErasure."""

        self.capture_queries = enabled
        if not enabled:
            self.last_query_heads = None

    def score_reference_embedding(
        self,
        reference_embedding: torch.Tensor,
        batch_size: int = 1,
    ) -> torch.Tensor:
        """Score image tokens against one pooled reference concept.

        Args:
            reference_embedding: One pooled concept embedding with shape
                ``[kv_dim]`` or ``[1, kv_dim]``. The original implementation
                performs text-token pooling before this attention computation.
            batch_size: Number of conditional prompt samples to score. The
                legacy ScaleErasure path uses the first conditional sample.

        Returns:
            Per-head scores with shape ``[batch_size, query_tokens, heads]``.
        """

        if self.last_query_heads is None:
            raise RuntimeError("query capture must be enabled before scoring references")
        if reference_embedding.ndim == 1:
            reference_embedding = reference_embedding.unsqueeze(0)
        if reference_embedding.ndim != 2 or reference_embedding.shape[0] != 1:
            raise ValueError(
                "reference_embedding must contain exactly one pooled concept "
                "with shape [kv_dim] or [1, kv_dim]"
            )

        reference_embedding = reference_embedding.to(
            device=self.mat_kv.weight.device,
            dtype=self.mat_kv.weight.dtype,
        )
        projected_kv = F.linear(reference_embedding, self.mat_kv.weight, None)
        projected_kv = projected_kv.view(1, 2, self.num_heads, self.head_dim)
        # The paper's token relevance uses normalized external concept keys,
        # matching the original ``compute_external_attention(..., norm=True)``
        # implementation.
        reference_key = F.normalize(projected_kv[:, 0], dim=-1).float()
        query_heads = self.last_query_heads[:batch_size].float()
        token_scores = torch.einsum(
            "blhd,shd->blhs", query_heads, reference_key
        )
        return token_scores[..., 0].mul(self.scale)
